...
func (cc *ClusterCreate) check() error {
	...
	if _, ok := cc.mutation.DisplayName(); !ok {
		return &ValidationError{Name: "display_name", err: errors.New(`ent: missing required field "Cluster.display_name"`)}
	}
	if _, ok := cc.mutation.NumNodes(); !ok {
		return &ValidationError{Name: "num_nodes", err: errors.New(`ent: missing required field "Cluster.num_nodes"`)}
	}
	if v, ok := cc.mutation.NumNodes(); ok {
